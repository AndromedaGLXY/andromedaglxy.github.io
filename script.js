...
// MindSight Trainer: Colours & Shapes (full screen, audio cues)
...
function renderOptions(){selectArea.innerHTML='';const list=(mode==='colours')?COLOURS:SHAPES;list.forEach(item=>{
 ...
 c.addEventListener('change',()=>{if(c.checked){if(chosen.length>=6){c.checked=false;alert('Choose up to 6.');return;}chosen.push(item);}else{chosen=chosen.filter(x=>x!==item);}});
...
function showGame(){if(chosen.length===0){alert('Please choose at least 1 item (up to 6).');return;}menu.classList.add('hidden');setup.classList.add('hidden');game.classList.remove('hidden');revealed=false;nextItem();}
